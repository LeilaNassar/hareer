<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 py-8 md:py-10">
        <div class="mb-8">
            ...
            <p class="text-gray-500 mt-1">Update product details, images, stock, and size guide.</p>
        </div>
...
Height 165-170 cm | Suitable weight 65-75 kg"
                >{{ old('size_guides_text', $product->size_guides ? implode("\n", $product->size_guides) : '') }}</textarea>
            ...
            <div>
                <label class="block mb-1 font-medium">Current Main Image</label>

                @if ($product->image)
                    <img
                        src="{{ asset('storage/' . $product->image) }}"
                        alt="{{ $product->name }}"
                        class="w-28 h-28 md:w-32 md:h-32 object-cover rounded mb-3"
                    >
                @else
            ...
            <div>
                <label class="block mb-2 font-medium">Add More Product Images</label>
